...
resource "aws_lb_listener_rule" "images" {
  ...
  priority     = 10
  action { type = "forward"; target_group_arn = aws_lb_target_group.tg_images.arn }
  condition { path_pattern { values = ["/images/*"] } }
}

resource "aws_lb_listener_rule" "register" {
  listener_arn = aws_lb_listener.http.arn
  priority     = 20
  action { type = "forward"; target_group_arn = aws_lb_target_group.tg_register.arn }
  condition { path_pattern { values = ["/register/*"] } }
}
